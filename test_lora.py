...
with fine-tuned LoRA model from HuggingFace.
"""
...
# Configuration - same hyperparameters as base model
CONFIG = Img2ImgConfig(
    ...
    seed=42,
    use_ilvr=False,
    use_cls_conditioning=False,  # Disabled for this run
    ip_adapter_scale=0.6,
)

# LoRA configuration (downloaded locally)
LORA_DIR = "lora_weights/checkpoint-100/lora"
LORA_SCALE = 1.0  # Scale for LoRA weights
...
def main():
    print("=" * 70)
    print("VALIDATION SET EVALUATION - IMG2IMG WITH CLS + LORA FINETUNED")
    print("=" * 70)
    
    # Setup paths
    output_dir = Path("evals/lora_finetuned_no_cls")
    ...
    print(f"\nLoRA Model: {LORA_DIR} (scale={LORA_SCALE})")
    ...
    # Load LoRA weights using PEFT (trained with PEFT format)
    print(f"\nLoading LoRA weights from {LORA_DIR}...")
    pipeline.pipe.unet = PeftModel.from_pretrained(
        pipeline.pipe.unet,
        LORA_DIR,
        adapter_name="finetuned",
    )
    pipeline.pipe.unet.set_adapter("finetuned")
    print(f"LoRA weights loaded and activated with scale={LORA_SCALE}!")
    ...
    for i, pair in enumerate(tqdm(val_pairs, desc="Generating")):
        ...
        # Generate with CLS conditioning + LoRA
        print(f"  Generating with img2img + CLS + LoRA...")
